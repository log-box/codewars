...
# of the sequence to generate the next. And, worse part of it, regrettably I won't get to hear non-native
...
def tribonacci(signature, n):
    ...
    if n > 2:
        while len(_array) < n:
            e = ((_array[len(_array)-1] + _array[len(_array)-2] + _array[len(_array)-3]))
            _array.append(e)
        return _array
    elif n == 0:
        return []
