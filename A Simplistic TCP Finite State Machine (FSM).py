...
#
# The input will be an array of events. Your job is to traverse the FSM as determined by the events, and return the
# proper state as a string, all caps, as shown above.
#
...
def checksStateEvent(state, event):
    if state in states:
        if event in states[state]:
            return True
        else:
            return False
    else:
...
#
# This kata is similar to Design a Simple Automaton (Finite State Machine), and you may wish to try that kata before
# tackling this one.
#
